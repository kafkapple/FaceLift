...
def create_labeled_input_strip(
    ...
    view_indices: list = None,   # NEW: Tensor position -> camera ID mapping
) -> np.ndarray:
    """
    ...
    Args:
        ...
        view_indices: List mapping tensor position to camera ID.
                      e.g., [3, 1, 5, 0, 2, 4] means all_images[0] is camera 3.
                      If None, assumes all_images[i] is camera i (identity mapping).
    ...
    # If view_indices is provided, use it; otherwise assume identity
    ...
    for cam_idx in camera_order:
        ...
        if tensor_idx is not None and tensor_idx < num_views:
            img = all_images[tensor_idx, :3, ...]  # [C, H, W] - FIXED: use tensor_idx
