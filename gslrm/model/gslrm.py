...
class Renderer(nn.Module):
    ...
    def _render_sequential(
        ...
        Returns:
            tuple: (renderings, alphas) where both are [b, v, c, h, w]
        """
...
class LossComputer(nn.Module):
    ...
    def _create_visual_inline(self, rendering, target, v, mask=None, rendered_alpha=None):
        ...
        - Row 4: Rendered + Pred Mask overlay (RGB threshold)
        - Row 5: Error map
        - Row 6 (NEW): Alpha mask comparison at thresholds 0.1, 0.3, 0.5, 0.7
        - Row 7 (NEW): RGB pred mask comparison at thresholds 0.05, 0.1, 0.2, 0.3
        """
    ...
    def _create_threshold_comparison(self, rendering, rendered_alpha, gt_mask, v, h):
        """
        Create threshold comparison visualization.
        
        Shows masks at different thresholds for both alpha and RGB methods.
        Row 1: Alpha thresholds (0.1, 0.3, 0.5, 0.7) - 4 views worth
        Row 2: RGB thresholds (0.05, 0.1, 0.2, 0.3) - 4 views worth
        Row 3: GT mask, Best Alpha, Best RGB, Diff (if gt_mask available)
        """
        import numpy as np
        
        device = rendering.device
        b_times_v = rendering.shape[0]
        _, c, h_img, w_img = rendering.shape
        
        # Only use first batch item for visualization
        render_sample = rendering[0:v]  # [v, 3, h, w]
        alpha_sample = rendered_alpha[0:v] if rendered_alpha is not None else None  # [v, 1, h, w]
        gt_mask_sample = gt_mask[0:v] if gt_mask is not None else None  # [v, 1, h, w]
        
        # Alpha thresholds
        alpha_thresholds = [0.1, 0.3, 0.5, 0.7]
        ...
        # Row 1: Alpha threshold masks (show first view with different thresholds)
        ...
        rgb_row = []
        bg_white = 1.0
        color_dist = (render_sample[0:1] - bg_white).abs().mean(dim=1, keepdim=True)  # [1, 1, h, w]
        ...
        if gt_mask_sample is not None and alpha_sample is not None:
            comparison_row = []
            ...
            gt_only = (gt_binary > 0.5) & (alpha_binary < 0.5)
            both = (alpha_binary > 0.5) & (gt_binary > 0.5)
            
            diff_img = torch.zeros(1, 3, h_img, w_img, device=device)
        ...
        # Ensure all rows have same width by padding
...
class GSLRM(nn.Module):
    ...
    def forward(
        ...
        if target_data is not None:
            ...
            # Render images using Gaussian splatting (now returns tuple with alpha)
            rendered_images, rendered_alpha = self.gaussian_renderer(
                xyz, features, scaling, rotation, opacity,
        ...
        if loss_metrics is not None:
            ...
            # Add ghosting metrics (NEW - for monitoring ghost artifacts)
            if rendered_alpha is not None:
                # Alpha foreground coverage (ghost = low coverage or scattered)
                loss_metrics.ghost_fg_coverage = (rendered_alpha > 0.5).float().mean()
                # Alpha variance (ghost = high variance in alpha)
                loss_metrics.ghost_alpha_std = rendered_alpha.std()
            
            # Opacity statistics from Gaussian parameters
            if opacity is not None:
                loss_metrics.ghost_opacity_std = opacity.std()
                loss_metrics.ghost_opacity_mean = opacity.mean()
