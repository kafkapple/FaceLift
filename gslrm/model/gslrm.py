...
class LossComputer(nn.Module):
    ...
    def _compute_mask_iou(self, rendering, target, gt_mask, rendered_alpha=None):
        """Compute IoU between GT mask and predicted mask. Delegates to mouse_extensions."""
        return compute_mask_iou(
            rendering=rendering,
            gt_mask=gt_mask,
            config=self.config,
    ...
    def _add_error_scale_annotation(self, visual_np, error_stats, row_height, num_rows):
        """Add error scale annotation. Delegates to mouse_extensions."""
        return add_error_scale_annotation(visual_np, error_stats, row_height, num_rows)
...
class GSLRM(nn.Module):
    ...
    def save_validation_results(
        ...
        Delegates to mouse_extensions.validation.ValidationRunner for modularity.
        """
        from mouse_extensions.validation import run_validation
        return run_validation(
            self, output_directory, model_results, batch_data, dataset, save_visualizations
        )
