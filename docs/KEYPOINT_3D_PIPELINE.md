...
> **Created**: 2026-03-04 | **Version**: v1.1
...
4. Add Gaussian noise sigma = {0, 1, 2, 5} px to simulate detection error
5. Triangulate -> compare with GT

### Key Insight

By comparing DANNCE's real 6-view MPJPE (Section 9) with synthetic N-view results
at various noise levels, we can estimate DANNCE's effective detection noise sigma.
...
| File | Description |
|------|-------------|
| `multiview_results.json` | view_count x noise_level -> MPJPE table |
...
| Raw Cameras | `~/data/raw/markerless_mouse_1_nerf/new_cam.pkl` | 6 x {K, R, t} |
...
- Mapping: `dannce_frame = m5_frame x 5`
...
All cameras are virtual turntable cameras (elevation=20 deg, radius=2.7, 384px).
Noise sigma = {0.5, 1, 2, 3, 5, 10} px applied uniformly.

#### Exp 2: Real 6-cam Baseline
Same noise levels applied to the 6 physical cameras (1152x1024).
...
| Turntable 72v | 72 | 0.85 | approx match |
...
1. **Theoretical scaling**: MPJPE proportional to sigma/sqrt(N) with ~15-20% geometry bonus
   - Log-log slope approx -0.55 (vs -0.50 theoretical)
   - Empirical always beats 1/sqrt(N) due to parallax improvement

2. **Camera quality matters more than quantity**:
   - Real 6 cameras (0.82mm) approx 72 turntable cameras (0.85mm)
   - Resolution (1152x1024 vs 384) + 3D angular diversity -> 4.4x advantage
...
4. **Saturation point** (turntable, sigma=5px):
...
5. **DANNCE gap analysis**:
   - DANNCE real (6.15mm) vs Oracle sigma=5px (0.82mm) -> 5.33mm gap
   - Gap = calibration error + body model mismatch + detection bias
   - Implies DANNCE effective noise >> sigma=5px (closer to sigma=40px or systematic)

### Literature Context

- DLT triangulation error bound: O(sigma/sqrt(N)) (Hartley & Zisserman, 2003)
...
| `plots/02_theoretical_fit.png` | Empirical vs 1/sqrt(N) theory |
