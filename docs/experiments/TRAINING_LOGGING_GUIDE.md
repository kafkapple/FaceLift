...
training:
  schedule:
    early_stop_patience: 10  # validation 개선 없이 N회 연속 시 종료
```
...
| 항목 | 값 | 설명 |
|------|-----|------|
| **patience** | 10 | val/psnr이 10회 연속 개선 안 되면 종료 |
| **val_every** | 200 steps | ~2000 steps without improvement |
| **기본 동작** | 활성화 | patience > 0 이면 자동 활성화 |

### 로그 예시

```
[EarlyStopping] New best PSNR: 22.3456
[EarlyStopping] No improvement. Patience: 1/10
[EarlyStopping] No improvement. Patience: 2/10
...
[EarlyStopping] Triggered! Best PSNR: 22.3456
```
...
학습 종료 시 자동으로 `best_psnr.pt` 로드 후 test set 평가:

```
[Test] Loading best checkpoint: checkpoints/gslrm/.../best_psnr.pt
[Test] Best checkpoint loaded (step 3200)
```
...
또는 temporal split 데이터셋 사용 시 자동 활성화 (M5t, M0t 등)
