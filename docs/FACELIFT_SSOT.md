...
PS 방법론 오분류 + `13.78/0.846` 재현 불가. **전문 = §2.1 C5 블록**, 재현성 검사 = §5.1, 복구계획 = `Obsidian:30_Projects/FaceLift/_Agent/260724_ps_baseline_STATUS.md`.
...
| **D15** | ✅ fixed 260724 | 파이프라인 조사 | `fix_m5_camera_params.py` = 소실된 convert 의 카메라부 완전 대체 확인. 재산출 계획 = `Obsidian:30_Projects/FaceLift/_Agent/260724_ps_baseline_STATUS.md`. 잔여 격차 = 이미지→zarr |
