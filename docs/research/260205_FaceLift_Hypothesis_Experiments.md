...
## 13. View Ablation 실험 (260205) ⭐⭐⭐

### 13.1 목적

GS-LRM 입력 뷰 개수의 최적값 탐색 (현재 4-view baseline)

### 13.2 Inference-time 결과 (기존 4-view 모델 사용)

| Views | 선택 인덱스 | PSNR ↑ | SSIM | 비고 |
|-------|------------|--------|------|------|
| **3** | [0, 2, 4] (120°) | **21.12±0.79** | 0.6051 | 🥇 최고 |
| **2** | [0, 3] (180°) | **20.20±0.69** | 0.6052 | 🥈 |
| 4 | baseline | 19.58±0.90 | 0.6048 | 기준선 |
| 5 | [0,1,2,3,4] | 19.58±0.90 | 0.6048 | 4와 동일 |
| 6 | 전체 | 19.58±0.90 | 0.6048 | 4와 동일 |

### 13.3 핵심 발견

**예상과 반대 결과:**
- 3-view가 4-view보다 **+1.54 PSNR** 우수
- 2-view도 4-view보다 **+0.62 PSNR** 우수
- 5, 6-view는 4-view와 동일 (모델 내부적으로 4개만 사용?)

**해석 가능성:**
1. **균등 분포 효과**: 120° 간격 3-view가 정보 효율적
2. **Overfitting to 4-view**: 4-view 학습 시 특정 패턴에 과적합
3. **Redundancy**: 인접 뷰는 중복 정보

### 13.4 Training-time 실험 (진행 중)

| 조건 | GPU | Config | WandB | 상태 |
|------|-----|--------|-------|------|
| 2-view | 6 |  |  | 🔄 |
| 3-view | 5 |  |  | 🔄 |
| 4-view | - | baseline |  | ✅ 완료 |
...
| Views | 선택 인덱스 | PSNR | SSIM | 비고 |
|-------|------------|--------|------|------|
| 3 | [0, 2, 4] (120 deg) | 21.12 | 0.6051 | 최고 |
| 2 | [0, 3] (180 deg) | 20.20 | 0.6052 | 2위 |
| 4 | baseline | 19.58 | 0.6048 | 기준선 |
| 5 | [0,1,2,3,4] | 19.58 | 0.6048 | 4와 동일 |
| 6 | 전체 | 19.58 | 0.6048 | 4와 동일 |

### 13.3 핵심 발견

- 3-view가 4-view보다 +1.54 PSNR 우수
- 2-view도 4-view보다 +0.62 PSNR 우수
- 균등 분포 (120 deg 간격)가 효과적
