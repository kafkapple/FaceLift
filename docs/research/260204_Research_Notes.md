# 260204 Research Notes

> **Date**: 2026-02-04
> **Topics**: 4D Gaussian Splatting, Temporal Consistency Methods

---

## 1. Executive Summary

4D Gaussian Splatting의 temporal consistency 확보 전략 3가지:
...
| 방법 | 핵심 아이디어 | 장점 | 단점 |
|------|--------------|------|------|
| D-3DGS | Canonical + Deform MLP | 구현 단순 | Canonical 품질 의존 |
| Dynamic 3DGS | Per-frame + Rigidity | Dense tracking | 큰 변형 불가 |
| SC-GS | Sparse control points | 최고 품질, 편집 가능 | 초기화 민감 |
| 4D-GS | HexPlane decomposition | Real-time | 긴 시퀀스 메모리 |
| Native 4D | 4D ellipsoids | 유연성 | 계산 비용 |
...
```
L_rigidity = Sum_i Sum_j in N(i) ||v_i - R*v_j||²
```
인접 Gaussian들의 motion이 locally rigid하도록 강제

### 4.2 ARAP (As-Rigid-As-Possible) Loss
```
L_ARAP = Sum_i Sum_j in N(i) ||p\'_j - p\'_i - R_hat_i(p_j - p_i)||²
```
SC-GS에서 사용, 생쥐의 관절 움직임에 적합

### 4.3 Velocity Smoothness
```
L_velocity = ||v_t - v_{t-1}||²
```
속도 연속성 강제

### 4.4 Optical Flow Alignment
```
L_flow = w * ||Delta_p_2D - flow||² (uncertainty-weighted)
```
...
|----------|--------|--------|
| P0 | temporal_regularization.py | Medium |
| P1 | temporal_pipeline_v2.py | High |
| P2 | optical_flow_loss.py | Medium |
| P3 | sliding_window_trainer.py | High |
...
*FaceLift Research Notes | 2026-02-04*
